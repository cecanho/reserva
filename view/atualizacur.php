...
<div class="w3-center">
    ...
    <div id="meudiv">
    <form method="post" action="atualizarcur.php">
        <input type="hidden" name="id" value="<?php echo $_POST['id'];?>"></div><br>
        <?php
        $rs = $con->prepare('SELECT * FROM curso WHERE id = ?');
        ...
        if($rs->execute()){
            if($rs->rowCount() > 0){
                while($row = $rs->fetch(PDO::FETCH_OBJ)){
                    ...
                    echo '<div><label>Área:</label></div>';
                    echo '<div><input type="text" name="area" id="area" value="'.$row->area.'"></div>';
                }
            }
        }
        ?>
