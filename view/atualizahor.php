...
<div class="w3-center">
    ...
    <div id="meudiv">
        <form method="post" action="atualizarhor.php">
            <input type="hidden" name="id" value="<?php echo $_POST['id'];?>"></div><br>
            <?php
            $rs = $con->prepare('SELECT * FROM horario WHERE id = ?');
            $rs->bindParam(1, $_POST['id']);
            if($rs->execute()){
                if($rs->rowCount() > 0){
                    while($row = $rs->fetch(PDO::FETCH_OBJ)){
                        echo '<div><label>Horário:</label></div>';
                        echo '<div><input type="text" name="horario" id="horario" value="'.$row->horario.'"></div>';
                    }
                }
            }
            ?>
